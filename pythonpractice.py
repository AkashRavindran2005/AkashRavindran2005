...
def debt():
    new = Toplevel(root)
    new.title('Debt repayment calculator')
    label0 = tk.Label(new, text='Date')
    label = tk.Label(new, text='Current balance')
    label1 = tk.Label(new, text='Monthly payment')
    label2 = tk.Label(new, text='Interset rate')
    entry0 = tk.Entry(new, textvariable=tk.StringVar())
    entry = tk.Entry(new, textvariable=tk.IntVar())
    entry1 = tk.Entry(new, textvariable=tk.IntVar())
    entry2 = tk.Entry(new, textvariable=tk.StringVar())
    ...
    def submit():
        ...
        ir = entry2.get()
        print(type(ir))
        log1 = -1*(1 - (float(cb)*float(ir) / float(mp)))
        log2 = 1 + float(ir)
        st = numpy.log(log1)
        sec = numpy.log(log2)
        repay = (st/sec)
        date = d[:-1] + str(int(d[-1])+int(repay)) 
        messagebox.showinfo('Debt free by', 'Debt free by '+date)
    submi = tk.Button(new, text='Submit', command=submit)
